...
def update_config(body: dict = Body(...)):
    # Accept partial or extra fields from the frontend to avoid 422 errors.
    ...
    if "prefer_bluetooth" in body:
        val = body["prefer_bluetooth"]
        if isinstance(val, str):
            cfg["prefer_bluetooth"] = val.strip().lower() in {"1", "true", "yes", "on"}
        else:
            cfg["prefer_bluetooth"] = bool(val)
